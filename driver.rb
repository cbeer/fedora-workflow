...
Dir.glob('participants/*.rb').each { |x| p "requiring " + x; require x }
...
client.subscribe(activemq_topic) do |msg|
  begin
    ...
    dsid = t.to_s unless t.nil?
    dsid ||= ''
      params= { :msg => msg, :pid => pid, :action => action, :dsid => dsid }.merge(Hash[*doc.xpath('//atom:category', 'atom' => 'http://www.w3.org/2005/Atom').map { |c| [c.xpath('@scheme').to_s, c.xpath('@term').to_s ] }.flatten] )
...
amp;dsid=' + CGI::escape(dsid)
    ...
    doc.xpath('//process-definition').each do |wkfw|
      $engine.launch wkfw.to_s, params
    end
  rescue Exception => e
    #log
    puts e
